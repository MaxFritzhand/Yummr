...
class CameraViewModel: ObservableObject {
    ...
    enum CaptureMode {
        ...
        case automatic2(everySecs: Double)
        
        
        /// The user has selected automatic capture mode, which captures one
        /// image every specified interval.
        case automatic3(everySecs: Double)
    }
    ...
    @Published var captureMode: CaptureMode = .manual {
        willSet(newMode) {
            ...
            if case .automatic2 = captureMode {
                stopAutomaticCapture()
                triggerEveryTimer = nil
            };
            if case .automatic3 = captureMode {
                stopAutomaticCapture()
                triggerEveryTimer = nil
            }
        }
        ...
        didSet {
            ...
            // the timer.
            if case .automatic2(let intervalSecs) = captureMode {
                ...
                triggerEveryTimer = TriggerEveryTimer(
                    ...
                    updateEvery: 1.0 / 30.0,  // 30 fps.  /// this will need to be fixed for smoothness likely **
                    onUpdate: { timeLeft in
                        self.timeUntilCaptureSecs = timeLeft
                    })
            }; if case .automatic3(let intervalSecs) = captureMode {
                autoCaptureIntervalSecs = intervalSecs
                triggerEveryTimer = TriggerEveryTimer(
                    triggerEvery: autoCaptureIntervalSecs,
                    onTrigger: {
    ...
    static let defaultAutomaticCaptureInterval_2Secs: Double = 2.0
    
    static let defaultAutomaticCaptureInterval_3Secs: Double = 3.0
...
//        switch captureMode{
//            case .manual:
//            captureMode = .automatic2(everySecs: <#T##Double#>)
//            case .automatic2(everySecs: let everySecs):
//                captureMode =
//            case .automatic3(everySecs: let everySecs):
//                captureMode = pauseSession()
//        }
//
//    }
    ...
    func advanceToNextCaptureMode() {
        ...
        switch captureMode {
        case .manual:
            captureMode = .automatic2(everySecs: CameraViewModel.defaultAutomaticCaptureInterval_2Secs)
        case .automatic2(_):
            captureMode = .automatic3(everySecs: CameraViewModel.defaultAutomaticCaptureInterval_3Secs)
        case .automatic3(_):
            captureMode = .manual
        }
    }
    ...
    func updateFocus(withCoordinate coordinate: DragGesture.Value) {
        let device = videoDeviceInput!.device
                do {
                    try device.lockForConfiguration()
                    if device.isFocusPointOfInterestSupported {
                        device.focusPointOfInterest = CGPoint(x: coordinate.location.x, y: coordinate.location.y)
                        device.exposurePointOfInterest = CGPoint(x: coordinate.location.x, y: coordinate.location.y)
